...
(library (libc libc)
  ...
  (define load-object
    (lambda (dir name)
      (cond
        [(file-exists? (format "./~a/~a" dir name))
          (load-shared-object (format "./~a/~a" dir name))]
        [(file-exists? (format "./lib/~a/~a" dir name))
          (load-shared-object (format "./lib/~a/~a" dir name))]
        [else (load-shared-object name)])))

  (define lib (load-object "libc" lib-name))
